...
<%-- 					<c:if test = "${result == 0}">
						<div class = "check_warn">유효하지 않은 사번입니다.</div>
					</c:if> --%>
					</div>
					<p class="order_box" style=" text-align: center;">
						<a class="true"><button type="submit" class="check_button agree_button"
							onclick="submit_check();">인증하기</button></a>
						<!-- <a href="#" onclick="submit_check();">인증하기</a>-->
					</p>
				</form>
		</div>
		</section>
...
<script>
	...
	function submit_check() {
		...
		//var birthday = "";
/* 		var birthday_yyyy = document.getElementById("birthday_yyyy").value;
		var birthday_mm = document.getElementById("birthday_mm").value;
		var birthday_dd = document.getElementById("birthday_dd").value; */
...
/* 	 */
	...
	/* 		if (!birthday_mm.value) {
	 alert("월을 선택하세요.");
	 birthday_mm.focus();
	 return false;
	 }
	 if (!birthday_dd.value) {
	 alert("일을 선택하세요.");
	 birthday_dd.focus();
	 return false;
	 } */
	...
	/* 			alert("인증되었습니다.");	
	 ...
	 birthday_mm = (birthday_mm < 10 ? '0' : '') + birthday_mm;
	 birthday_dd = (birthday_dd < 10 ? '0' : '') + birthday_dd;
	...
	/*  	$(".check_button").click(function(){
	 $("#join_check").attr("action", "/join_agree");
	 $("#join_check").submit();
	 })  */
</script>
